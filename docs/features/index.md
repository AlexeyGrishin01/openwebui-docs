...
- 📅 **Prompt Variables Support**: Prompt variables such as ``, ``, ``, ``, ``, ``, ``, ``, and `` can be utilized in the system prompt or by using a slash command to select a prompt directly within a chat.
...
- 🔄 **Seamless Integration**: Copy any `ollama run ` CLI command directly from a model's page on [Ollama library](https://ollama.com/library/) and paste it into the model dropdown to easily select and pull models.
