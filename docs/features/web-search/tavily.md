...
For a comprehensive list of all environment variables related to Web Search (including concurrency settings, result counts, and more), please refer to the [Environment Configuration documentation](../getting-started/env-configuration#web-search.md).
...
- **Open WebUI Installed**: A running instance of Open WebUI (local or Docker). See the [Getting Started guide](getting-started).
...
- **WEBUI_URL Environment Variable**: Configured to point to your Open WebUI instance. Refer to [Environment Variable Configuration](environment).
...
- **Environment Variables**: Set the `TAVILY_API_KEY` in your `.env` file or Docker command:
  ...
  See [Environment Variable Configuration](environment).
- **RAG Integration**: Combine Tavily results with local data using Retrieval Augmented Generation. Refer to [RAG Documentation](features/rag).
...
- [Open WebUI Features](features): Details on RAG and web search.
- [Contributing to Open WebUI](contributing): Share improvements or report issues.
