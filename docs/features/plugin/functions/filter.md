...
<img src="assets/images/toggle-filter.png" alt="Toggle Filter">
...
**API Endpoint:**
```http
POST /functions/id//toggle/global
```
...
class Filter:
    class Valves(BaseModel):
        selected_theme: str = Field(
            ...
            json_schema_extra=, # KEY: This creates the dropdown
        )
        ...
        colorize_type: str = Field(
            ...
            json_schema_extra=, # Another example of an enum dropdown
        )
    ...
    def outlet(self, body: dict, __user__: Optional[dict] = None) -> dict:
        ...
        for message in body["messages"]:
            if message["role"] == "assistant":  # Target model response
                message["content"] = f"****"  # Highlight with Markdown
        return body
```
