...
4. The base64 string is **replaced with a file URL** (e.g., `/api/v1/files//content`)
...
The model's code should print the base64 data URL. Open WebUI intercepts this and converts it to a permanent file URL. The model should then use this **resulting URL** in markdown like `<img src="api/v1/files/abc123/content.md" alt="Chart">` — it should NOT paste the raw base64 string into its response text.
...
3. Open WebUI converts this to a file URL in the output (e.g., `<img src="api/v1/files/abc123/content.md" alt="Output Image">`)
