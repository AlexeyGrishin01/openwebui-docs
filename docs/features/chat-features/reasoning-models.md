...
| Anthropic (extended thinking) | ❌ Not supported — Anthropic requires structured `` blocks, use a pipe function |
...
**Cause:** This is a fundamental architectural difference. Open WebUI follows the **OpenAI Chat Completions API standard** and does not natively support Anthropic's proprietary API format. Anthropic's extended thinking requires structured content blocks with `` or ``, which are Anthropic-specific formats that don't exist in the OpenAI standard.
