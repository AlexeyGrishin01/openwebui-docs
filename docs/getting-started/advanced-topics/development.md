...
**Prefer Docker?** If setting up a local dev environment seems like too much, you can also test by running the [dev Docker image](#using-the-dev-branch-.md) — it's the same code, just containerized.
...
-   Check the [Environment Configuration](getting-started/env-configuration.md#cors_allow_origin) guide for details on how to set `CORS_ALLOW_ORIGIN` to allow requests from your frontend's URL (e.g., `http://localhost:5173`).
