...
The `REDIS_KEY_PREFIX` allows multiple Open WebUI instances to share the same Redis instance without key conflicts. In Redis cluster mode, the prefix is formatted as `:` (e.g., `:config:*`) to enable multi-key operations on configuration keys within the same hash slot.
...
- [Open WebUI Environment Variables](getting-started/env-configuration/)
