...
*   **Kubernetes / Helm**: For deploying on K8s with multiple replicas, see the **[Multi-Replica & High Availability Guide](troubleshooting/multi-replica.md)**.
*   **Redis (Mandatory)**: When running multiple workers (`UVICORN_WORKERS > 1`) or multiple replicas, **Redis is required** to handle WebSocket connections and session syncing. See **[Redis Integration](tutorials/integrations/redis.md)**.
*   **Load Balancing**: Ensure your Ingress controller supports **Session Affinity** (Sticky Sessions) for best performance.
*   **Reverse Proxy Caching**: Configure your reverse proxy (e.g., Nginx, Caddy, Cloudflare) to **cache static assets** (JS, CSS, Images). This significantly reduces load on the application server. See **[Nginx Config](tutorials/https/nginx.md)** or **[Caddy Config](tutorials/https/caddy.md)**.
*   **Disable Proxy Buffering (Critical for Streaming)**: If using Nginx, you **must** disable `proxy_buffering` for Open WebUI. Proxy buffering re-chunks SSE streams, causing garbled markdown and slow streaming. Add `proxy_buffering off;` and `proxy_cache off;` to your location block. See **[Streaming Troubleshooting](troubleshooting/connection-error.md#-garbled-markdown--streaming-response-corruption)**.
...
For detailed information on all available variables, see the [Environment Configuration](getting-started/env-configuration.md) guide.

| Variable | Description & Link |
| :--- | :--- |
| `TASK_MODEL` | [Task Model (Local)](getting-started/env-configuration.md#task_model) |
| `TASK_MODEL_EXTERNAL` | [Task Model (External)](getting-started/env-configuration.md#task_model_external) |
| `ENABLE_BASE_MODELS_CACHE` | [Cache Model List](getting-started/env-configuration.md#enable_base_models_cache) |
| `MODELS_CACHE_TTL` | [Model Cache TTL](getting-started/env-configuration.md#models_cache_ttl) |
| `ENABLE_QUERIES_CACHE` | [Queries Cache](getting-started/env-configuration.md#enable_queries_cache) |
| `DATABASE_URL` | [Database URL](getting-started/env-configuration.md#database_url) |
| `ENABLE_REALTIME_CHAT_SAVE` | [Realtime Chat Save](getting-started/env-configuration.md#enable_realtime_chat_save) |
| `CHAT_RESPONSE_STREAM_DELTA_CHUNK_SIZE` | [Streaming Chunk Size](getting-started/env-configuration.md#chat_response_stream_delta_chunk_size) |
| `THREAD_POOL_SIZE` | [Thread Pool Size](getting-started/env-configuration.md#thread_pool_size) |
| `RAG_EMBEDDING_ENGINE` | [Embedding Engine](getting-started/env-configuration.md#rag_embedding_engine) |
| `AUDIO_STT_ENGINE` | [STT Engine](getting-started/env-configuration.md#audio_stt_engine) |
| `ENABLE_IMAGE_GENERATION` | [Image Generation](getting-started/env-configuration.md#enable_image_generation) |
| `ENABLE_AUTOCOMPLETE_GENERATION` | [Autocomplete](getting-started/env-configuration.md#enable_autocomplete_generation) |
| `RAG_SYSTEM_CONTEXT` | [RAG System Context](getting-started/env-configuration.md#rag_system_context) |
| `DATABASE_ENABLE_SESSION_SHARING` | [Database Session Sharing](getting-started/env-configuration.md#database_enable_session_sharing) |
